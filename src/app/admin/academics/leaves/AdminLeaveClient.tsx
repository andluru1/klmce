...
export default function AdminLeaveClient({ leaves, currentUserId }: { leaves: any[], currentUserId: string }) {
  ...
  const handleStatus = async (leaveId: string, action: 'ADMIN_APPROVE' | 'REJECT') => {
    setLoading(true);
          ...
          >
            <div className="flex justify-between items-start mb-2">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${
                leave.status === 'HOD_APPROVED' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 
            ...
            <p className="text-[10px] text-slate-500 mt-2">
              {leave.faculty.department?.name}
            </p>
          </GlassCard>
        ))}
        {leaves.length === 0 && (
          <div className="p-8 text-center text-slate-500 border border-dashed border-slate-800 rounded-xl">
            No HOD-approved leaves waiting for final decision.
          </div>
        )}
      </div>
        ...
        {activeLeave ? (
          <GlassCard className="p-0 flex flex-col h-[600px] border-slate-800">
            <div className="p-6 border-b border-slate-800 flex justify-between items-start bg-slate-900/50">
              ...
              {activeLeave.status === 'HOD_APPROVED' && (
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => handleStatus(activeLeave.id, 'ADMIN_APPROVE')}
                    disabled={loading}
            ...
            <div className="p-4 bg-slate-900 border-t border-slate-800">
              <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                <input 
                  ...
                  placeholder="Message the HOD and applicant..."
